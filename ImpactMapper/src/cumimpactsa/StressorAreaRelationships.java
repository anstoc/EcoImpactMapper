...
public class StressorAreaRelationships
{
    private class DataItem
    {
        ...
        int y;
        double divIndex;
        double[] stressorValues; //order like MappingProject.stressors
    }
    ...
    public StressorAreaRelationships()
    {
        ...
        MappingProject.processingProgressPercent = 10;
        
        //make list of data for sorting by div. index.
        double[][] divData=index.grid.getData();
        for(int x=0; x<MappingProject.grid.getDimensions().x;x++)
        {
            for(int y=0; y<MappingProject.grid.getDimensions().y; y++)
            {
                ...
                //item.stressorNames=new String[MappingProject.stressors.size()];
                item.stressorValues=new double[MappingProject.stressors.size()];
                boolean noData=false;
                for(int s=0; s<MappingProject.stressors.size(); s++)
                {
                    item.stressorValues[s]=MappingProject.stressors.get(s).getGrid().getData()[x][y];
       ...
       MappingProject.processingProgressPercent = 15;
       Collections.sort(items, new ItemComparator());
       MappingProject.processingProgressPercent = 20;    
       ...
       percentStressors=new double[MappingProject.stressors.size()][items.size()+1];
       double[] stressorSum = new double[MappingProject.stressors.size()];
       ...
       for(int i=1; i<=items.size();i++)
       {
           MappingProject.processingProgressPercent = (int) (20+80*i/items.size());
           //System.out.println("Calculation done: " + MappingProject.processingProgressPercent);
           for(int s=0; s<items.get(i-1).stressorValues.length; s++)
           {
    ...
    public void writeToFile(String filename)
    {
        ...
        for(int s=0; s<MappingProject.getStressorNames().length; s++)
        {
            table.addColumn(MappingProject.getStressorNames()[s]);
        }
